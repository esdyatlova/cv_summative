    ...
    for i in range(len(contours)):
        ...
        if hierarchy[0][i][3] == 0:  # определение контура буквы
            if len(letters) != 0 and h > 20:
                if (y_previous - y) > h:  # определение строки, на которой буква располагается
                    line += 1
                y_previous = y
            letters.append((x, line, np.array(im[y:y + h, x:x + w]),
                            w))  # добавление буквы в список букв [координата по х, номер строки, изображение буквы, ширина буквы]
...
def internal_contours(im):  # подсчет количества внутренних контуров у буквы
    ...
    contours_all, i = cv2.findContours(thresh, cv2.RETR_LIST,
                                       cv2.CHAIN_APPROX_SIMPLE)  # подсчет количества всех контуров
    contours_ex, i2 = cv2.findContours(thresh, cv2.RETR_EXTERNAL,
                                       cv2.CHAIN_APPROX_SIMPLE)  # подсчет количества внешних контуров

    return str(len(contours_all) - len(
        contours_ex))  # количество внутренних контуров = количество всех контуров - количество внешних контуров
...
def vertical_lines(im):  # подсчет количества вертикальных линий у буквы
    ...
    for j in range(
            w - 1):  # для каждого столбца в массиве хранящем изображение найдем среднее значение пикселей и сравним его с пороговым
...
def horizontal_lines(im):  # подсчет количества горизонтальных линий у буквы
    ...
    h_lines = [np.array([])]
    for i in range(
            h - 1):  # для каждой строки в массиве хранящем изображение найдем среднее значение пикселей и сравним его с пороговым
...
def ts_tsh(im):  # Ц или Щ
    h, w = im.shape
    im = (
    im[0:10, (w // 2 - 5):(w // 2 + 5)])  # проверяем попадает ли буква в небольшую область на самом верху изображения
...
def m_i(im):  # М или И
    h, w = im.shape
    im = (im[(h - 10):h,
          (w // 2 - 5):(w // 2 + 5)])  # проверяем попадает ли буква в центральный нижний сектор (М - да, И - нет)
...
def defining_letter(incnt, vnum, hnum, fvline, fhline, lhline, lvline, tstsh, ze, lp, mi,
                    uh):  # функция распознавания, получает на вход значения всех функций определения контуров буквы, возвращает букву
